...
5. Dans Railway, ouvrir uniquement le projet `m3s-budget-preview-20260904`, service `m3s-backend-preview`; sous `Settings > Source`, selectionner `snchgroup-web/m3s-backend`, branche `main`, puis sous `Deployments` exiger le commit `5abd8df142065a11a631490c440328c752fe8cdd` avant de poursuivre.
...
7. Pour la seconde instance, utiliser `New > GitHub Repo`, choisir le meme depot et la meme branche, nommer exactement le service `m3s-backend-preview-b`, recopier par l'interface Railway les seules variables du service preview, generer un domaine Railway et consigner son URL. Stopper si la revision, le dataset ou une variable non sensible differe.
8. Apres avoir consigne le domaine du second service dans `$secondary`, executer la recette HTTP sur chaque instance uniquement avec les six variables `BUDGET_HTTP_*` chargees dans le processus courant : `npm run budget:http:check -- --execute --non-production --url https://m3s-backend-preview-production.up.railway.app/api --confirm https://m3s-backend-preview-production.up.railway.app/api`, puis la meme commande en remplacant deux fois l'URL par `$secondary/api`. Conserver ensuite uniquement les deux rapports nettoyes; chaque instance doit produire exactement un `409` controle.
...
13. Pour chacune des dix phases groupees nommees ci-dessus, consigner `$startUtc` avant la premiere action et `$endUtc` apres les cinq minutes d'observation. Sans enregistrer les journaux bruts, analyser chaque service avec `npx @railway/cli logs --http --service m3s-backend-preview --environment production --since $startUtc --until $endUtc --json | npm run budget:preview:logs -- --http --expected-409 0` et la meme commande avec `--service m3s-backend-preview-b`. Chaque analyse doit contenir au moins vingt sondes `/health` propres au service. Pour les deux recettes HTTP de l'operation 8, utiliser `--expected-409 1`. Tout code retour `2` impose `STOP`, retour arriere et relais immediat du marqueur dans le canal confirme.
14. Pour chaque phase contenant une requete Budget, verifier aussi le contrat de journal nettoye avec `npx @railway/cli logs --service m3s-backend-preview --environment production --since $startUtc --until $endUtc --filter '@event:budget_request' --json | npm run budget:preview:logs -- --application --expected-revision 5abd8df142065a11a631490c440328c752fe8cdd`, puis la meme commande pour `m3s-backend-preview-b`. L'analyse doit retrouver exactement les dix champs autorises avec leurs types, valeurs bornees, route Budget normalisee et revision autorisee; aucune sortie brute n'est conservee.
15. Pour le frontend, creer dans `snchgroup-web/m3s-frontend-v2` la branche exacte `codex/budget-p3-p4-preview-rollback-20260906` depuis `e2141df`; ouvrir une PR vers `main` afin d'obtenir une seule URL Deploy Preview. Le premier commit ne change que le commentaire de cadrage et conserve `REACT_APP_BUDGET_STORAGE_ENABLED=true`; consigner le SHA et l'URL avant test.
16. Cycle de retour arriere 1 : modifier uniquement `netlify.toml` en remplacant `REACT_APP_BUDGET_STORAGE_ENABLED = "true"` par `"false"`, pousser sur la meme branche et attendre le meme Deploy Preview en `200`; mettre ensuite uniquement `FINANCE_BUDGET_DRAFTS_ENABLED=false` sur les deux services backend preview et redeployer. Avec frontend et backend simultanement fermes, verifier absence des actions serveur, export JSON disponible, `enabled:false`, ecriture refusee, sante et journaux conformes. Seulement apres ce constat complet, remettre le backend a `true`, puis le frontend a `true`, redeployer les trois cibles et verifier leur etat actif avant le cycle 2. Ne jamais publier le frontend comme production.
17. Cycle de retour arriere 2 : repeter dans le meme ordre la fermeture du frontend preview, puis de chacun des deux backends preview. Verifier de nouveau, pendant que les trois cibles sont simultanement fermees, absence des actions serveur, export JSON disponible, `enabled:false`, ecriture refusee, sante et journaux conformes. Ne rien rouvrir apres cette seconde preuve : le frontend temporaire reste ferme jusqu'a la fermeture de sa PR et les deux backends restent obligatoirement avec `FINANCE_BUDGET_DRAFTS_ENABLED=false` pour l'operation 18.
18. Fin de fenetre : conserver d'abord `FINANCE_BUDGET_DRAFTS_ENABLED=false`; restaurer sur le service principal le `M3S_AUTH_USERS_JSON` fictif initial; renommer `JWT_SECRET_PREVIEW_ROLLBACK` en `JWT_SECRET` sans afficher ni changer sa valeur; mettre `M3S_AUTH_SIGNING_MODE=legacy`, redeployer et exiger sante `200`; retirer ensuite `M3S_AUTH_SIGNING_KEYS_JSON` et `M3S_AUTH_SIGNING_MODE`, redeployer et exiger encore sante `200`. Supprimer alors `m3s-backend-preview-b`, fermer sans fusion la PR frontend temporaire et verifier une derniere fois refus d'ecriture, secret de developpement absent et production inchangee.
...
- Au premier critere d'arret : cesser les tests et mettre `FINANCE_BUDGET_DRAFTS_ENABLED=false`. Si `JWT_SECRET_PREVIEW_ROLLBACK` existe sur le service principal, le renommer d'abord en `JWT_SECRET` sans lire ni changer sa valeur, passer `M3S_AUTH_SIGNING_MODE=legacy`, redeployer et exiger sante `200`; retirer seulement ensuite `M3S_AUTH_SIGNING_KEYS_JSON` et `M3S_AUTH_SIGNING_MODE`, redeployer et exiger encore sante `200`. Restaurer le compte fictif initial, supprimer le service temporaire, verifier refus d'ecriture et absence du secret de developpement, puis classer `RETOUR ARRIERE`. Si cette restauration protegee echoue, arreter le service principal au lieu de retirer le trousseau.
