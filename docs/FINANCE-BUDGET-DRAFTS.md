...
18. Fin de fenetre : conserver d'abord `FINANCE_BUDGET_DRAFTS_ENABLED=false`; restaurer sur le service principal le `M3S_AUTH_USERS_JSON` fictif initial. Si `JWT_SECRET_PREVIEW_ROLLBACK` existe, le renommer en `JWT_SECRET` sans afficher ni changer sa valeur; sinon exiger que `JWT_SECRET` existe deja. Mettre `M3S_AUTH_SIGNING_MODE=legacy`, redeployer et exiger sante `200`; retirer ensuite `M3S_AUTH_SIGNING_KEYS_JSON` et `M3S_AUTH_SIGNING_MODE`, redeployer et exiger encore sante `200`. Supprimer alors `m3s-backend-preview-b`, fermer sans fusion la PR frontend temporaire, verifier les capacites authentifiees `enabled:false`, puis lancer sur le seul service principal `npm run budget:preview:health -- --execute --non-production --phase FINAL_CLEANUP --url https://m3s-backend-preview-production.up.railway.app/api --confirm https://m3s-backend-preview-production.up.railway.app/api` et controler la matrice applicative `200:1`. Exiger enfin secret de developpement absent et production inchangee.
