...
13. Pour chaque phase, consigner `$startUtc` avant la mutation et `$endUtc` apres les cinq minutes d'observation. Sans enregistrer les journaux bruts, analyser chaque service avec `npx @railway/cli logs --http --service m3s-backend-preview --environment production --since $startUtc --until $endUtc --json | npm run budget:preview:logs -- --http --expected-409 0` et la meme commande avec `--service m3s-backend-preview-b`. Pour les deux recettes HTTP de l'operation 8, utiliser `--expected-409 1`. Tout code retour `2` impose `STOP`, retour arriere et relais immediat du marqueur dans le canal confirme.
