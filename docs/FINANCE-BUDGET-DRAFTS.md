...
14. Pour chaque phase contenant une requete Budget, verifier aussi le contrat de journal nettoye avec `npx @railway/cli logs --service m3s-backend-preview --environment production --since $startUtc --until $endUtc --filter '@event:budget_request' --json | npm run budget:preview:logs -- --application`, puis la meme commande pour `m3s-backend-preview-b`. L'analyse doit trouver uniquement les champs autorises, une route Budget normalisee et la revision presente; aucune sortie brute n'est conservee.
...
18. Fin de fenetre : restaurer sur le service principal le `M3S_AUTH_USERS_JSON` fictif initial, supprimer `m3s-backend-preview-b`, retirer `M3S_AUTH_SIGNING_KEYS_JSON`, `M3S_AUTH_SIGNING_MODE` et le trousseau temporaire, conserver `FINANCE_BUDGET_DRAFTS_ENABLED=false`, fermer sans fusion la PR frontend temporaire et verifier une derniere fois sante, refus d'ecriture et production inchangee.
