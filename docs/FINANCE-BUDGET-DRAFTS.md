...
| Duree candidate | 120 minutes maximum a compter de l'heure de debut consignee |
...
- Fenetre totale : 120 minutes maximum, heure de debut et heure de fin cible consignees avant la premiere mutation.
- Observation : un controle toutes les 15 secondes pendant cinq minutes pour une fenetre commune `HTTP_ACCEPTANCE`, puis apres chacune des dix phases groupees `LEGACY_BASELINE`, `PRIMARY_SHARED`, `SECONDARY_SHARED`, `NEW_ACTIVE_BOTH`, `OLD_REMOVED_BOTH`, `WRITE_REMOVED_BOTH`, `OWNER_DISABLED_BOTH`, `ROLLBACK_1_CLOSED`, `ROLLBACK_2_CLOSED` et `FINAL_CLEANUP`, soit onze fenetres et 55 minutes d'observation planifiees. Les deux services d'une meme phase partagent la meme fenetre; aucune observation supplementaire n'est exigee entre eux. Des blocs `DEPLOYMENT_GUARD` consecutifs couvrent en plus le service principal depuis `$authorizationStartUtc`, y compris le premier deploiement et la restauration finale; ils n'acceptent que `$originalRevision` et la revision epinglee. La vie du service secondaire est horodatee avant sa creation, puis protegee des que son domaine existe par `SECONDARY_DEPLOYMENT_GUARD`, qui n'accepte que la revision epinglee.
...
| Phase | Statuts exacts attendus par service | Code exact des `401` |
| --- | --- | --- |
| `HTTP_ACCEPTANCE` | `200:3,201:1,401:1,404:2,409:1` | `null` |
| `LEGACY_BASELINE` | `200:1` | `none` |
| `PRIMARY_SHARED` | `200:2` | `none` |
| `SECONDARY_SHARED` | `200:3` | `none` |
| `NEW_ACTIVE_BOTH` | `200:3` | `none` |
| `OLD_REMOVED_BOTH` | `200:1,401:3` | `null` |
| `WRITE_REMOVED_BOTH` | `200:1,403:1` | `none` |
| `OWNER_DISABLED_BOTH` | `401:1` | `BUDGET_UNAUTHENTICATED` |
| `ROLLBACK_1_CLOSED` | `200:1` | `none` |
| `ROLLBACK_2_CLOSED` | `200:1` | `none` |
| `FINAL_CLEANUP` | `200:1` sur le seul service principal encore vivant | `none` |
...
2. Controler les deux cibles avant mutation avec `Invoke-WebRequest` sur `https://m3s-backend-preview-production.up.railway.app/api/health` et `https://deploy-preview-327--m3s-frontend-v2.netlify.app`; exiger `200`. Relever sans valeur sensible le depot, la branche source, l'identifiant du deploiement, la revision et la valeur `NODE_ENV` initiaux du service principal dans `$originalSourceRepo`, `$originalSourceBranch`, `$originalDeploymentId`, `$originalRevision` et `$originalNodeEnv`; toute valeur absente interdit la suite. Consigner ensuite `$authorizationStartUtc` immediatement avant l'operation 3 et emettre dans les trente secondes un nouveau `GET /api/health` controle, dont la revision doit etre `$originalRevision`; cette requete est le marqueur HTTP verifiable du debut d'audit. Cette borne ouvre la duree maximale globale de 120 minutes. La fenetre probatoire epinglee `$runStartUtc` ne commence qu'apres le deploiement epingle de l'operation 6, mais la garde de deploiement couvre sans interruption toute mutation du service principal depuis `$authorizationStartUtc` jusqu'a `$authorizationEndUtc`.
3. Avant toute mutation, authentifier une fois la CLI Railway avec `npx @railway/cli login`, puis lier uniquement la cible avec `npx @railway/cli link --project m3s-budget-preview-20260904 --environment production --service m3s-backend-preview`. Executer `npm run budget:preview:logs -- --self-test-alert`; le code retour `2` et le marqueur `BUDGET_PREVIEW_STOP` doivent etre relayes dans la tache Codex courante et acquittes par Cheikh ou Chantal. Sans cet acquittement, ne pas commencer. Aussitot l'acquittement recu et avant la premiere mutation, lancer dans une console dediee des blocs consecutifs `npm run budget:preview:health -- --execute --non-production --phase DEPLOYMENT_GUARD --url https://m3s-backend-preview-production.up.railway.app/api --confirm https://m3s-backend-preview-production.up.railway.app/api --allowed-revisions "$originalRevision,5abd8df142065a11a631490c440328c752fe8cdd"`. Conserver le `startUtc` du premier rapport comme `$primaryGuardStartUtc`. Chaque bloc exige vingt reponses completes, mesure la latence apres lecture du JSON et n'accepte que ces deux revisions distinctes; ne laisser aucun intervalle entre blocs et ne pas poursuivre au moindre echec.
...
5. Depuis un clone propre de `snchgroup-web/m3s-backend`, verifier que `git rev-parse 5abd8df142065a11a631490c440328c752fe8cdd^{commit}` retourne exactement ce SHA, puis creer la branche distante temporaire epinglee avec `git push origin 5abd8df142065a11a631490c440328c752fe8cdd:refs/heads/codex/budget-p3-p4-authorized-revision-20260906`. Pendant que les blocs `DEPLOYMENT_GUARD` restent actifs, ouvrir uniquement dans Railway le projet `m3s-budget-preview-20260904`, service `m3s-backend-preview`; sous `Settings > Source`, selectionner `snchgroup-web/m3s-backend` et cette branche temporaire. Sous `Deployments`, exiger le commit complet `5abd8df142065a11a631490c440328c752fe8cdd` avant de poursuivre; toute autre revision impose `STOP`.
6. Dans `Variables`, utiliser l'editeur du service pour mettre `NODE_ENV=development`, ajouter `M3S_AUTH_SIGNING_KEYS_JSON` et `M3S_AUTH_SIGNING_MODE=legacy`, sans modifier les autres noms. Refuser `NODE_ENV=production` ou toute autre valeur. Sous `Deployments`, lancer `Deploy` et attendre `/api/health = 200` avec la revision exacte, toujours sous `DEPLOYMENT_GUARD`. Attendre la fin du bloc de garde courant, consigner son `endUtc` comme `$runStartUtc`, puis enchainer sans pause dans la meme console des blocs `npm run budget:preview:health -- --execute --non-production --phase RUN_GUARD --url https://m3s-backend-preview-production.up.railway.app/api --confirm https://m3s-backend-preview-production.up.railway.app/api --expected-revision 5abd8df142065a11a631490c440328c752fe8cdd` jusqu'a l'arret explicite de la garde avant la restauration de source de l'operation 18. Chaque bloc dure exactement cinq minutes; ne pas en commencer un dont la fin depasserait `$authorizationStartUtc + 120 minutes`.
7. Pour la seconde instance, consigner `$secondaryCreationStartUtc` immediatement avant de cliquer sur `New > GitHub Repo`, puis choisir le meme depot et exactement la branche epinglee `codex/budget-p3-p4-authorized-revision-20260906`, nommer le service `m3s-backend-preview-b`, recopier par l'interface Railway les seules variables du service preview, generer un domaine Railway et consigner son URL sans attendre un premier succes de sante. Conserver l'identifiant et le statut nettoyes de chaque build/deploiement Railway depuis `$secondaryCreationStartUtc`; tout etat `failed`, `crashed` ou `cancelled`, toute revision differente ou historique incomplet impose `STOP`. Des que le domaine existe, consigner `$secondaryStartUtc` puis lancer dans les trente secondes, dans une autre console, des blocs consecutifs `npm run budget:preview:health -- --execute --non-production --phase SECONDARY_DEPLOYMENT_GUARD --url $secondary/api --confirm $secondary/api --expected-revision 5abd8df142065a11a631490c440328c752fe8cdd`; le premier appel de ce bloc est le marqueur HTTP du debut d'audit et son `startUtc` devient `$secondaryGuardStartUtc`. Le premier statut autre que `200`, corps incomplet ou SHA different impose `STOP`. Exiger sur les deux services le SHA complet `5abd8df142065a11a631490c440328c752fe8cdd`; stopper aussi si le dataset ou une variable non sensible differe. Apres le premier bloc reussi, enchainer pour le service secondaire les memes blocs `RUN_GUARD` sans pause jusqu'a sa suppression. L'analyse integrale de l'operation 18 couvre toute sa vie HTTP depuis `$secondaryStartUtc`; l'historique Railway controle separement la creation anterieure au domaine.
8. Apres avoir consigne le domaine du second service dans `$secondary`, ouvrir une seule fenetre commune `HTTP_ACCEPTANCE` de cinq minutes. Lancer simultanement, dans deux consoles distinctes, les sondes `npm run budget:preview:health -- --execute --non-production --phase HTTP_ACCEPTANCE --url <URL-SERVICE>/api --confirm <URL-SERVICE>/api --expected-revision 5abd8df142065a11a631490c440328c752fe8cdd` pour le principal et le secondaire. Sans attendre leur fin, lancer simultanement dans deux autres consoles la recette HTTP avec les six variables `BUDGET_HTTP_*` chargees dans chaque processus : `npm run budget:http:check -- --execute --non-production --url https://m3s-backend-preview-production.up.railway.app/api --confirm https://m3s-backend-preview-production.up.railway.app/api` et la meme commande avec `$secondary/api`. Les identifiants de run sont aleatoires et les brouillons synthetiques restent distincts. Les deux rapports de sonde doivent partager une fenetre se chevauchant pendant les cinq minutes et fournissent leurs propres `startUtc` et `endUtc`; conserver uniquement les rapports nettoyes. Chaque instance doit produire exactement la matrice `200:3,201:1,401:1,404:2,409:1` et un seul `409` controle.
9. Dans une console persistante distincte, charger seulement `BUDGET_HTTP_OWNER_EMAIL` et `BUDGET_HTTP_OWNER_PASSWORD`, puis lancer `npm run budget:preview:transitions -- --execute --non-production --primary-url https://m3s-backend-preview-production.up.railway.app/api --secondary-url $secondary/api --confirm "https://m3s-backend-preview-production.up.railway.app/api|$secondary/api" --expected-revision 5abd8df142065a11a631490c440328c752fe8cdd`. Cette commande conserve les trois jetons uniquement en memoire, refuse les hotes de production, exige la revision autorisee sur les deux santes pendant chaque echantillon, attend les six confirmations exactes ci-dessous et emet pour chaque phase son `startUtc` et son `endUtc` apres les cinq minutes completes.
10. A l'invite `PRIMARY_SHARED`, modifier uniquement le service principal en `M3S_AUTH_SIGNING_MODE=shared`, conserver les deux cles et `JWT_SECRET`, redeployer et exiger la sante `200`; saisir ensuite `CONFIRM PRIMARY_SHARED`. Le script doit se connecter separement aux deux services, exiger `kid=preview-old` sur le jeton emis par le principal et l'absence de `kid` sur celui emis par le secondaire, puis verifier le jeton du principal sur les deux instances. A l'invite `SECONDARY_SHARED`, appliquer la meme modification au second service, redeployer et saisir `CONFIRM SECONDARY_SHARED`. Le script doit alors se connecter au second service, exiger `kid=preview-old` dans le jeton emis et verifier ce jeton sur les deux instances. Le jeton historique sans `kid` et les deux jetons `preview-old` doivent rester utilisables sur les deux instances.
11. A l'invite `NEW_ACTIVE_BOTH`, remplacer uniquement `activeKeyId` par `preview-new` dans le meme trousseau sur les deux services, redeployer chacun et saisir `CONFIRM NEW_ACTIVE_BOTH`. Le script doit se connecter separement a chaque service, exiger `kid=preview-new` dans les deux jetons emis et verifier chacun de ces jetons sur les deux instances. A l'invite `OLD_REMOVED_BOTH`, retirer la cle `preview-old` et renommer uniquement la variable protegee `JWT_SECRET` en `JWT_SECRET_PREVIEW_ROLLBACK` sur les deux services sans afficher ni changer sa valeur; redeployer puis saisir `CONFIRM OLD_REMOVED_BOTH`. Le script doit alors refuser sur les deux instances le jeton historique et chacun des deux jetons `preview-old`, tout en acceptant le jeton `preview-new`.
...
13. Pour la fenetre commune `HTTP_ACCEPTANCE` et chacune des dix phases groupees nommees ci-dessus, reprendre exclusivement les bornes `startUtc` et `endUtc` emises pendant son execution par le runner de transitions ou le sondeur autonome; ne jamais les reconstruire apres coup. Sans enregistrer les journaux bruts, analyser chaque service encore vivant avec `npx @railway/cli logs --http --service m3s-backend-preview --environment production --since $startUtc --until $endUtc --json | npm run budget:preview:logs -- --http --expected-409 0 --start-utc $startUtc --end-utc $endUtc` et, lorsqu'il existe, la meme commande avec `--service m3s-backend-preview-b`. Chaque ligne HTTP doit contenir `timestamp`, `httpStatus`, `totalDuration` et `path`; chaque analyse doit contenir au moins vingt sondes `/health`, couvrir les deux bornes a trente secondes pres et ne presenter aucun intervalle de sante superieur a trente secondes. Pour la fenetre commune `HTTP_ACCEPTANCE` de l'operation 8, utiliser `--expected-409 1` sur chacun des deux services avec ses propres bornes. Ces controles de phase ne remplacent pas le controle HTTP integral de l'operation 18. Tout code retour `2` impose `STOP`, retour arriere et relais immediat du marqueur dans le canal confirme.
14. Pour chaque phase contenant une requete Budget, verifier aussi le contrat et le compte exact des statuts avec `npx @railway/cli logs --service m3s-backend-preview --environment production --since $startUtc --until $endUtc --filter '@event:budget_request' --json | npm run budget:preview:logs -- --application --expected-revision 5abd8df142065a11a631490c440328c752fe8cdd --expected-statuses <MATRICE-DE-LA-PHASE> --expected-401-code <CODE-401-DE-LA-PHASE>`, puis la meme commande pour `m3s-backend-preview-b` lorsqu'il existe. Remplacer les deux valeurs entre chevrons par les valeurs exactes de la matrice fermee ci-dessus, sans espace : `none` si aucun `401` n'est attendu, `null` pour un refus produit en amont par le validateur JWT, et `BUDGET_UNAUTHENTICATED` pour le compte fictif desactive. L'analyse doit retrouver les dix champs autorises, la route normalisee, la revision autorisee, exactement chaque statut attendu et le couple statut/code metier propre a la phase. Les succes `200/201` exigent `code:null`; les autres refus Budget exigent leur code deterministe. Aucune sortie brute n'est conservee.
...
16. Cycle de retour arriere 1 : modifier uniquement `netlify.toml` en remplacant `REACT_APP_BUDGET_STORAGE_ENABLED = "true"` par `"false"`, pousser sur la meme branche et attendre le meme Deploy Preview en `200`; mettre ensuite uniquement `FINANCE_BUDGET_DRAFTS_ENABLED=false` sur les deux services backend preview et redeployer. Avec frontend et backend simultanement fermes, verifier l'absence des actions d'ecriture frontend et l'export JSON disponible, sans provoquer de requete d'ecriture backend. Dans une premiere console par backend, lancer `npm run budget:preview:health -- --execute --non-production --phase ROLLBACK_1_CLOSED --url <URL-SERVICE>/api --confirm <URL-SERVICE>/api --expected-revision 5abd8df142065a11a631490c440328c752fe8cdd`; pendant cette fenetre et depuis une seconde console, verifier une seule fois les capacites authentifiees `enabled:false`. Utiliser les bornes du sondeur pour controler journaux HTTP et matrice applicative `200:1`. Seulement apres ce constat complet, remettre le backend a `true`, puis le frontend a `true`, redeployer les trois cibles et verifier leur etat actif avant le cycle 2. Ne jamais publier le frontend comme production.
17. Cycle de retour arriere 2 : repeter dans le meme ordre la fermeture du frontend preview, puis de chacun des deux backends preview. Pendant que les trois cibles sont simultanement fermees, verifier de nouveau l'absence des actions d'ecriture frontend et l'export JSON disponible, sans provoquer de requete d'ecriture backend. Dans une premiere console par backend, lancer `npm run budget:preview:health -- --execute --non-production --phase ROLLBACK_2_CLOSED --url <URL-SERVICE>/api --confirm <URL-SERVICE>/api --expected-revision 5abd8df142065a11a631490c440328c752fe8cdd`; pendant cette fenetre et depuis une seconde console, verifier une seule fois les capacites authentifiees `enabled:false`. Utiliser les bornes du sondeur pour controler journaux HTTP et matrice applicative `200:1`. Ne rien rouvrir apres cette seconde preuve : le frontend temporaire reste ferme jusqu'a la fermeture de sa PR et les deux backends restent obligatoirement avec `FINANCE_BUDGET_DRAFTS_ENABLED=false` pour l'operation 18.
18. Fin de fenetre : conserver d'abord `FINANCE_BUDGET_DRAFTS_ENABLED=false`; restaurer sur le service principal le `M3S_AUTH_USERS_JSON` fictif initial. Si `JWT_SECRET_PREVIEW_ROLLBACK` existe, le renommer en `JWT_SECRET` sans afficher ni changer sa valeur; sinon exiger que `JWT_SECRET` existe deja. Mettre `M3S_AUTH_SIGNING_MODE=legacy`, redeployer et exiger sante `200`; retirer ensuite `M3S_AUTH_SIGNING_KEYS_JSON` et `M3S_AUTH_SIGNING_MODE`, restaurer `$originalNodeEnv`, redeployer et exiger encore sante `200`. Dans une premiere console, lancer sur le service principal `npm run budget:preview:health -- --execute --non-production --phase FINAL_CLEANUP --url https://m3s-backend-preview-production.up.railway.app/api --confirm https://m3s-backend-preview-production.up.railway.app/api --expected-revision 5abd8df142065a11a631490c440328c752fe8cdd`; pendant cette fenetre et dans une seconde console, verifier une seule fois les capacites authentifiees `enabled:false`. Utiliser les bornes du sondeur pour controler la matrice applicative `200:1`. Attendre ensuite la fin du bloc `RUN_GUARD` secondaire courant sans en relancer, prendre son `endUtc` comme `$secondaryEndUtc`, puis analyser toute sa vie HTTP avec `npx @railway/cli logs --http --service m3s-backend-preview-b --environment production --since $secondaryStartUtc --until $secondaryEndUtc --json | npm run budget:preview:logs -- --http --expected-409 1 --start-utc $secondaryStartUtc --end-utc $secondaryEndUtc --health-start-utc $secondaryGuardStartUtc --health-end-utc $secondaryEndUtc`; le rapport doit confirmer `auditStartCovered:true`. Rapprocher ce rapport de l'historique de creation Railway nettoye depuis `$secondaryCreationStartUtc`, qui doit etre complet et sans echec, puis supprimer `m3s-backend-preview-b` et fermer sans fusion la PR frontend temporaire. Sur le service principal, attendre la fin du bloc `RUN_GUARD` courant sans en relancer et prendre son `endUtc` comme `$runEndUtc`; cette borne ferme la surveillance strictement liee a `5abd8df142065a11a631490c440328c752fe8cdd`. Sans aucune pause apres ce bloc, lancer dans une autre console des blocs consecutifs `DEPLOYMENT_GUARD` avec `--allowed-revisions "$originalRevision,5abd8df142065a11a631490c440328c752fe8cdd"`. Pendant que cette nouvelle garde reste active, analyser toute la fenetre epinglee avec `npx @railway/cli logs --http --service m3s-backend-preview --environment production --since $runStartUtc --until $runEndUtc --json | npm run budget:preview:logs -- --http --expected-409 1 --start-utc $runStartUtc --end-utc $runEndUtc` et exiger le succes avant tout changement de source. Restaurer ensuite exactement `$originalSourceRepo` et `$originalSourceBranch`, redeployer `$originalDeploymentId`, puis verifier `$originalRevision`, `$originalNodeEnv`, sante `200` et stockage toujours ferme pendant que `DEPLOYMENT_GUARD` reste active. Attendre la fin du bloc courant, consigner son `endUtc` comme `$authorizationEndUtc`, puis analyser la fenetre integrale du service principal avec `npx @railway/cli logs --http --service m3s-backend-preview --environment production --since $authorizationStartUtc --until $authorizationEndUtc --json | npm run budget:preview:logs -- --http --expected-409 1 --start-utc $authorizationStartUtc --end-utc $authorizationEndUtc --health-start-utc $primaryGuardStartUtc --health-end-utc $authorizationEndUtc`; le rapport doit confirmer `auditStartCovered:true`. Cette analyse integrale doit couvrir les deux changements de revision et retrouver exactement l'unique `409` controle de l'acceptation; elle est requise en plus du rapport epingle. Seulement apres son succes, supprimer la branche backend epinglee avec `git push origin --delete codex/budget-p3-p4-authorized-revision-20260906`. Les trois rapports doivent confirmer zero `5xx`, aucun enregistrement incomplet, marqueurs de debut presents, aucune rupture de sondes superieure a trente secondes pendant les fenetres de cadence gouvernees, sante sans echec et seuils de latence. Tout ecart impose `ECHEC`. Exiger enfin source et `NODE_ENV` initiaux restaures, secret de developpement absent, production inchangee et duree totale `$authorizationEndUtc - $authorizationStartUtc` inferieure ou egale a 120 minutes.
...
- Depassement de 120 minutes, absence du responsable, de la suppleante ou du canal confirme, ou franchissement d'un seuil ci-dessus.
