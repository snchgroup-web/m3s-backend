...
7. Pour la seconde instance, consigner `$secondaryStartUtc` immediatement avant de cliquer sur `New > GitHub Repo`, puis choisir le meme depot et exactement la branche epinglee `codex/budget-p3-p4-authorized-revision-20260906`, nommer le service `m3s-backend-preview-b`, recopier par l'interface Railway les seules variables du service preview, generer un domaine Railway et consigner son URL sans attendre un premier succes de sante. Des que le domaine existe, lancer dans une autre console des blocs consecutifs `npm run budget:preview:health -- --execute --non-production --phase SECONDARY_DEPLOYMENT_GUARD --url $secondary/api --confirm $secondary/api --expected-revision 5abd8df142065a11a631490c440328c752fe8cdd`; conserver le `startUtc` du premier rapport comme `$secondaryGuardStartUtc`. Le premier statut autre que `200`, corps incomplet ou SHA different impose `STOP`. Exiger sur les deux services le SHA complet `5abd8df142065a11a631490c440328c752fe8cdd`; stopper aussi si le dataset ou une variable non sensible differe. Apres le premier bloc reussi, enchainer pour le service secondaire les memes blocs `RUN_GUARD` sans pause jusqu'a sa suppression. L'analyse integrale de l'operation 18 commence a `$secondaryStartUtc`, avant la creation, et couvre donc aussi les journaux emis avant que le domaine soit sondable; sa cadence de sante est evaluee separement a partir de `$secondaryGuardStartUtc`.
