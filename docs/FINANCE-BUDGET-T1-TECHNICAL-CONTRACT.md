...
23. Les cinq operations V2 ordinaires et la promotion respectent les chemins, charges, codes HTTP, enveloppes de succes et d'erreur, pagination, resume sans montant, indicateur `replayed` et rapport ferme definis par le contrat filaire ; aucune route T1 implicite n'est ajoutee.
...
25. Une promotion ne cree et ne lie un brouillon que si tous les controles sont `apparie`; sinon elle retourne le rapport nettoye et deterministe avec `422 BUDGET_PROMOTION_REVIEW_REQUIRED`. Une reprise reussie restitue le rapport immuable lie au resultat initial.
