...
Toutes les routes V2 exigent le Bearer courant et repondent avec `Cache-Control: no-store`. Elles relisent les permissions applicatives depuis le compte actif a chaque requete. Les routes statiques `/capabilities` et `/promotions/:sourceDraftId` sont enregistrees avant `/:id`. Aucun `DELETE`, partage, approbation ou changement de proprietaire n'existe dans T1.

| Methode / chemin sous `/api/finance/budget-drafts-v2` | Permissions courantes requises | Entree | Succes |
| --- | --- | --- | --- |
| `GET /capabilities` | `finance:read` | aucune requete de stockage | `200`, `{ success: true, contractVersion: 2, enabled, canWrite, scope: "organization", access: "owner-only", personalEnabled: false, reason }` |
| `GET /` | `finance:read` | `limit` entier `1..50`, defaut `20`; `offset` entier `0..10000`, defaut `0`; aucun autre parametre | `200`, `{ success: true, contractVersion: 2, data: summary[], hasMore }` |
| `GET /:id` | `finance:read` | UUID au format accepte par `ID_PATTERN`; aucun parametre de requete | `200`, `{ success: true, contractVersion: 2, data: { ...summary, budget, referenceSnapshots } }` |
| `POST /` | `finance:read` et `finance:write` | charge de creation V2 fermee definie ci-dessous | `201`, `{ success: true, contractVersion: 2, data: summary }` |
| `PUT /:id` | `finance:read` et `finance:write` | charge de mise a jour V2 fermee avec `expectedVersion` | `200`, `{ success: true, contractVersion: 2, data: summary }` |
| `POST /promotions/:sourceDraftId` | `finance:read` et `finance:write` | charge et cle idempotente definies dans la section Promotion | `201` a la creation ou `200` a la reprise, avec `{ success: true, contractVersion: 2, data: summary, replayed, report }` |

`GET /capabilities` est la seule route exemptee du refus `BUDGET_V2_DISABLED` et de toute dependance au stockage Budget. Pour une identite authentifiee disposant de `finance:read`, elle retourne toujours `200`, y compris lorsque T1 est ferme, avec `enabled: false`, `canWrite: false` et une `reason` generique. Lorsque T1 est ouvert, `canWrite` vaut `true` seulement si le compte courant dispose aussi de `finance:write`; cette indication n'accorde elle-meme aucun droit. Les autres routes retournent `503 BUDGET_V2_DISABLED` tant que T1 est ferme.
...
Cet exemple abrege illustre la forme et seulement les deux premieres periodes ; il n'est pas une charge valide tant que tous les `periodId` de l'exercice ne sont pas fournis. `ORG-2SG`, `FY-2SG-2027` et les autres identifiants ne deviennent recevables qu'apres resolution dans leurs referentiels actifs du tenant courant. Il n'existe aucun champ autonome `entityLabelSnapshot` dans le schema ferme : le serveur place le libelle canonique de l'organisation resolue dans `referenceSnapshots.identity.entityId.labelSnapshot` et en copie la meme valeur dans la colonne de resume `entity`, jamais depuis la charge cliente.

Le schema V2 est ferme : tout champ inconnu ou requis mais absent est refuse a chaque niveau. Une creation cliente porte exactement `contractVersion` et `budget` ; une mise a jour porte exactement `contractVersion`, `budget` et `expectedVersion`, entier de 1 a `999999`, pour la comparaison atomique. `contractVersion` vaut exactement `2`. `budget` exige exactement `title`, `identity`, `responsibilities`, `rate`, `rateSource`, `rateDate` et `rows`. `identity` exige les deux identifiants non nuls `entityId` et `fiscalYearId`. `responsibilities` exige `budgetOwnerAgentId` non nul et `controllerAgentId`, seul champ nullable de ce niveau. Chaque ligne exige exactement `id`, `label`, `kind`, `direction`, `currency`, `periodValues` et `dimensions` ; chaque valeur de periode exige exactement `periodId` et `value`. `dimensions` exige exactement les huit cles `functionId`, `teamId`, `agentId`, `countryId`, `portfolioId`, `dossierId`, `projectId` et `phaseId`, chacune explicitement `null` ou porteuse d'un identifiant. Aucune de ces cles ne peut etre omise ; les blocs serveur `referenceSnapshots` et `promotion` sont toujours refuses dans une charge cliente.

`budget.title` est une chaine non vide apres trim, bornee a 120 caracteres ; `budget.rows` est un tableau de 100 lignes maximum ; `row.id` et `row.label` sont des chaines non vides bornees respectivement a 64 et 120 caracteres. Les enumerations V1 sont conservees dans T1 : `kind` vaut uniquement `operating`, `investment` ou `financing` ; `direction` vaut uniquement `in` ou `out` ; `currency` vaut uniquement `CHF` ou `CFA`. L'enveloppe JSON serialisee reste bornee a 512 Kio. Le compteur serveur n'entre jamais dans le contenu metier. La version stockee `1000000` est terminale et reste lisible ; toute nouvelle mise a jour est refusee par `BUDGET_VERSION_LIMIT` avant calcul d'un compteur hors borne.
...
9. Une dimension absente est representee par la valeur explicite `null` sous sa cle obligatoire ; un montant ou un triplet de taux absent utilise les chaines vides prescrites. Aucune omission ne devient zero ni reference par defaut.
...
Les couples `path`, `result` et `code` admis sont fermes. Pour `identity.entityId`, `identity.fiscalYearId` et `responsibilities.budgetOwnerAgentId`, les couples sont `apparie / REFERENCE_RESOLVED`, `ambigu / REFERENCE_AMBIGUOUS` ou `introuvable / REFERENCE_NOT_FOUND`; `identity.entityId` admet en plus `incompatible / REFERENCE_STATE_INVALID`, `identity.fiscalYearId` admet `incompatible / CALENDAR_INCOMPATIBLE`, et `responsibilities.budgetOwnerAgentId` admet `incompatible / RESPONSIBILITY_INVALID`. Pour `budget.year`, seuls `apparie / YEAR_MATCHED` et `incompatible / YEAR_MISMATCH` sont admis. Pour `budget.rate`, seuls `apparie / RATE_COMPATIBLE`, `incompatible / VALUE_INCOMPATIBLE` et `incompatible / WHITESPACE_INCOMPATIBLE` sont admis. Pour chaque `rows[<ordinal>].periodValues`, seuls `apparie / PERIOD_VALUES_COMPATIBLE`, `incompatible / CALENDAR_INCOMPATIBLE`, `incompatible / VALUE_INCOMPATIBLE` et `incompatible / WHITESPACE_INCOMPATIBLE` sont admis. Tout autre chemin, code ou combinaison est invalide et ne peut etre emis.

Une promotion reussit uniquement lorsque tous les controles sont `apparie`. Le serveur renvoie alors le rapport avec le resume V2 et le conserve, sans modification, dans les metadonnees serveur de promotion et dans la liaison idempotente afin que toute reprise restitue le meme rapport. Une demande bien formee contenant au moins un controle non apparie retourne `422 BUDGET_PROMOTION_REVIEW_REQUIRED` avec le seul rapport nettoye en complement de l'enveloppe d'erreur ; elle ne cree aucun brouillon V2 et aucune liaison idempotente. Dans cette commande seulement, lorsqu'une source de referentiel est disponible mais qu'une reference cible fournie est absente, non visible ou hors tenant, le controle correspondant vaut `introuvable / REFERENCE_NOT_FOUND`; lorsqu'une cible visible et resolue echoue sur son cycle de vie, sa periode d'effet, le calendrier de l'exercice ou la responsabilite, le controle correspondant vaut le couple `incompatible` ferme defini ci-dessus. Ces controles participent au rapport nettoye au lieu de produire les erreurs ordinaires de reference ou de validation. Cette exception ne revele ni la valeur fournie, ni identifiant, candidat ou libelle et ne s'applique pas au brouillon V1 source, a une source de referentiel indisponible, ni aux routes V2 de creation, lecture ou mise a jour. Le rapport n'est pas expose par les autres routes metier.
...
```json
{
  "promotion": {
    ...
    "sourceLegacyEntityLabel": "Organisation historique saisie",
...
Lorsqu'aucune liaison n'existe encore pour `idempotencyKeyHash`, la cle est en premiere utilisation : le serveur relit le brouillon V1 source, son `version`, le tenant et l'auteur, puis resout et valide les references cibles. La promotion exige alors `version === expectedSourceVersion` ; sinon elle retourne `BUDGET_SOURCE_VERSION_CONFLICT` sans creation ni liaison. Le serveur calcule aussi une empreinte SHA-256 du JSON V1 canonique et persiste `sourceVersion`, `sourceContentDigest`, `sourceExportRevision`, `sourceLegacyEntityLabel`, `sourceLegacyYear`, `targetEntityId`, `targetFiscalYearId`, `targetBudgetOwnerAgentId` et le rapport reussi dans `promotion`. `sourceExportRevision` recopie exactement l'entier V1 `budget.revision`, compris entre `0` et `1000000` ; il reste une provenance historique immuable, n'est jamais confondu avec la version serveur V2 et n'est ni remis a zero ni reconstruit. `sourceLegacyEntityLabel` recopie exactement la chaine V1 `budget.entity`, sans trim ni rapprochement ; elle reste une provenance privee et n'alimente ni l'identite ni la colonne de resume V2, toutes deux issues du libelle canonique resolu. `budget.title` et le triplet de taux compatibles sont recopies sans reecriture. Pour chaque ligne V1, le serveur recopie exactement `id`, `label`, `kind`, `direction` et `currency`, transforme les douze positions `months` en douze objets `periodValues` relies aux `periodId` canoniques, et cree `dimensions` avec exactement les huit cles `functionId`, `teamId`, `agentId`, `countryId`, `portfolioId`, `dossierId`, `projectId` et `phaseId`, toutes initialisees a `null`. Il cree simultanement une entree `referenceSnapshots.rows[]` de meme `rowId`, dont les huit instantanes de dimensions sont aussi exactement `null`. Le contrat T1 ne cree pas de nouveau compteur d'export client V2 ; sa definition eventuelle appartient a un lot frontend distinct. L'empreinte canonique complete de la requete et l'identifiant V2 deterministe couvrent le tenant, l'auteur, le brouillon source, sa version, son empreinte et les trois identifiants cibles resolus, puis l'identifiant est encode dans la forme UUID V4 acceptee par le validateur partage. La specification des deux derivations, leurs vecteurs de test et le controle de collision appartiennent au micro-lot `T1-E` avant toute implementation.
...
| `BUDGET_V2_DISABLED` | `503` | Contrat T1 ferme dans l'environnement, sauf `GET /capabilities` qui decrit cet etat avec `200` |
...
| `BUDGET_REFERENCE_STATE_INVALID` | `422` | Reference visible et resolue, hors exercice ou responsabilite specialises ci-dessous, mais statut de cycle de vie ou periode d'effet non recevable pour l'operation |
...
Le serveur evalue ces familles dans cet ordre normatif et retourne le premier echec : syntaxe, authentification, capacite V2, permission, disponibilite du stockage, existence et portee du brouillon, disponibilite des referentiels, resolution et visibilite de chaque reference, validite de son cycle de vie et de sa periode d'effet, validite propre de l'exercice, validite des responsabilites, relations parentales, versions, puis regles propres a la promotion. Deux exceptions fermees s'appliquent : `GET /capabilities` decrit une capacite V2 fermee sans consulter le stockage ni retourner `BUDGET_V2_DISABLED`; et la premiere utilisation d'une promotion bien formee convertit l'absence, l'invisibilite ou l'invalidite d'une reference cible en controle ferme du rapport, puis en `422 BUDGET_PROMOTION_REVIEW_REQUIRED`. Hors de cette exception de promotion, une reference manquante utilise `BUDGET_REFERENCE_NOT_FOUND`, tandis qu'une reference visible et resolue dont le statut ou la periode d'effet est irrecevable utilise `BUDGET_REFERENCE_STATE_INVALID`; `BUDGET_FISCAL_YEAR_INVALID` et `BUDGET_RESPONSIBILITY_INVALID` restent les codes specialises pour des objets deja visibles et resolus, et `BUDGET_REFERENCE_RELATION_INVALID` s'applique seulement apres leur validation individuelle. Une ecriture devenue techniquement incertaine apres le debut de la transaction remplace toute erreur de stockage generique par `BUDGET_WRITE_UNCERTAIN`.
...
6. Une reference `restricted` est refusee sans reveler son existence tant qu'aucune politique Management active ne prouve l'acces de l'identite courante. Une reference visible et resolue, mais inactive, future ou expiree pour l'operation, retourne le code de cycle de vie general ou le code specialise de l'exercice ou de la responsabilite.
...
14. Le schema ferme refuse les champs inconnus, toute cle requise absente, toute nullabilite ou enumeration hors contrat, les tailles excessives et toute mise a jour sans `expectedVersion` valide.
...
17. Un `PUT` V2 dont `expectedVersion` est obsolete retourne `409 BUDGET_VERSION_CONFLICT` sans ecrasement ; `expectedVersion` s'arrete a `999999` et la version stockee `1000000` est terminale.
...
23. Les cinq operations V2 ordinaires et la promotion respectent les chemins, permissions, charges, codes HTTP, enveloppes de succes et d'erreur, pagination, resume sans montant, indicateur `replayed` et rapport ferme definis par le contrat filaire ; `GET /capabilities` retourne `200` sans stockage lorsque T1 est ferme, les lectures exigent `finance:read`, les ecritures exigent `finance:read` et `finance:write`, et aucune route T1 implicite n'est ajoutee.
24. Chaque ligne V1 promue possede douze `periodValues`, huit dimensions explicitement `null` et une entree d'instantane de ligne dont les huit dimensions sont `null`; aucune dimension n'est inventee depuis la source V1.
25. Une promotion ne cree et ne lie un brouillon que si tous les controles sont `apparie`; sinon elle retourne le rapport nettoye et deterministe avec `422 BUDGET_PROMOTION_REVIEW_REQUIRED`, y compris lorsqu'une reference cible est absente, invisible ou visible mais irrecevable par son cycle de vie. Une reprise reussie restitue le rapport immuable lie au resultat initial. Un identifiant V1 presente a une lecture ou mise a jour V2 reste indistinguable d'un brouillon absent et retourne `404 BUDGET_DRAFT_NOT_FOUND`.
