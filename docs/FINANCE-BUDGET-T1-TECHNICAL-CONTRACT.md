...
Le serveur evalue ces familles dans cet ordre normatif et retourne le premier echec : syntaxe, authentification, capacite V2, permission, disponibilite du stockage, existence et portee du brouillon, disponibilite des referentiels, resolution et visibilite de chaque reference, validite propre de l'exercice, validite des responsabilites, relations parentales, versions, puis regles propres a la promotion. Deux exceptions fermees s'appliquent : `GET /capabilities` decrit une capacite V2 fermee sans consulter le stockage ni retourner `BUDGET_V2_DISABLED`; et la premiere utilisation d'une promotion bien formee convertit l'absence ou l'invisibilite d'une reference cible en controle `introuvable / REFERENCE_NOT_FOUND` du rapport, puis en `422 BUDGET_PROMOTION_REVIEW_REQUIRED`. Hors de cette exception de promotion, une reference manquante utilise `BUDGET_REFERENCE_NOT_FOUND`; `BUDGET_FISCAL_YEAR_INVALID` et `BUDGET_RESPONSIBILITY_INVALID` ne s'appliquent qu'a des objets deja visibles et resolus, et `BUDGET_REFERENCE_RELATION_INVALID` seulement apres leur validation individuelle. Une ecriture devenue techniquement incertaine apres le debut de la transaction remplace toute erreur de stockage generique par `BUDGET_WRITE_UNCERTAIN`.
...
25. Une promotion ne cree et ne lie un brouillon que si tous les controles sont `apparie`; sinon elle retourne le rapport nettoye et deterministe avec `422 BUDGET_PROMOTION_REVIEW_REQUIRED`, y compris lorsqu'une reference cible est absente ou invisible. Une reprise reussie restitue le rapport immuable lie au resultat initial. Un identifiant V1 presente a une lecture ou mise a jour V2 reste indistinguable d'un brouillon absent et retourne `404 BUDGET_DRAFT_NOT_FOUND`.
