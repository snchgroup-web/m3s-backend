...
2. utilise dans T1-B un extracteur ferme et borne pour enumerer les references et les seules relations necessaires lorsqu'elles sont structurellement accessibles ; `MAX_DETAIL_REFERENCE_PAIRS` vaut exactement `804` et toute tentative d'enumerer un `805e` couple distinct retourne `BUDGET_STORAGE_UNAVAILABLE` avant tout appel de source ;
...
La frontiere de liste construit egalement une seule instance T1-B.1 propre a la requete. Son cache interne memorise, par couple `type + id`, uniquement le resultat independant du chemin fourni par le resolveur et les controles generiques de source, tenant, visibilite, cycle de vie et periode d'effet, dans ce tenant, pour cet acteur, en `READ` et au `requestAt` commun. `validateTypeRecord` est reexecute pour chaque chemin de responsabilite attendu et toutes les relations parent-enfant sont revalidees pour chaque occurrence ; leur succes ou leur erreur specialisee n'est jamais memoise par le seul couple `type + id`. Aucun resultat brut du cache n'est restitue et le cache est detruit a la fin de la requete. `MAX_LIST_REFERENCE_RESOLUTIONS` vaut exactement `4096` couples uniques : le `4097e` couple est refuse avant tout appel de source et toute la liste echoue en `503 BUDGET_STORAGE_UNAVAILABLE`, sans page partielle. Pour chaque brouillon, l'extracteur applique d'abord `MAX_DETAIL_REFERENCE_PAIRS = 804` et refuse le `805e` couple distinct avant toute resolution ; une lecture directe ne peut donc jamais depasser `804` appels de source, meme si l'enveloppe stockee est corrompue.
...
| `BUDGET_STORAGE_UNAVAILABLE` | `503` | stockage indispensable indisponible, ambigu ou corrompu ; tentative d'un `805e` couple distinct dans un brouillon ; sentinelle presente apres `10051` candidats sans page determinable ; tentative d'un `4097e` couple distinct dans la requete ; ou tentative de depasser `67108864` octets parcourus |
...
31. liste partageant un seul cache T1-B.1 entre candidats pour les seuls resultats independants du chemin, reexecutant `validateTypeRecord` selon le role attendu et les relations a chaque occurrence, continuant sur des cache hits apres `4096` couples et refusant seulement la tentative d'un `4097e` couple avant appel de source ;
...
33. extracteur direct et de liste refusant la tentative d'un `805e` couple distinct dans un brouillon avant tout appel de source, y compris lorsque le document corrompu contient plus de `100` lignes.
