...
test('row-security flags and policy definitions are part of catalog conformance', async () => {
  const schemaPlan = buildBankAccountPostgresOfflinePlan();
  const driftStatements = [
    `ALTER TABLE "${SCHEMA_NAME}"."${REVISION_TABLE}" ENABLE ROW LEVEL SECURITY`,
    `ALTER TABLE "${SCHEMA_NAME}"."${REVISION_TABLE}" FORCE ROW LEVEL SECURITY`,
    `CREATE POLICY bank_account_revision_probe
      ON "${SCHEMA_NAME}"."${REVISION_TABLE}" USING (true)`
  ];
